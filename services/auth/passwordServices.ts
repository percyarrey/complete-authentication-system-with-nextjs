...
export async function resetPassword(id:string, code:string, password:string) {
    await connectDB();
    ...
    if (passwordMatch) {
        return { success: false, message: 'Password must not be the same as the old password' };
    }
